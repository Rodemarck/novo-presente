...
public class ImageController {
    ...
    public CompletableFuture<ResponseEntity<byte[]>> mangaCover(@PathVariable int id) throws SQLException {
        return func(MangaDao.getImage(id));
    }

    @PostMapping("manga")
    public String postManga(@RequestParam("id") int id, @RequestParam("photo") MultipartFile photo) throws IOException {
        System.out.println("id = " + id);
        System.out.println("photo = " + photo.getSize());
        return "isso é um post caralho";
    }
}
